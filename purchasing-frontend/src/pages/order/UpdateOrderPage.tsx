...
import type { Product, OrderUpdateData, Order } from '../../types';
...
const UpdateOrderPage: React.FC = () => {
  ...
  const { control, handleSubmit, watch, reset, formState: { errors } } = useForm<OrderUpdateData>({
    defaultValues: {
      productOrders: [{
        product: '',
        quantity: 1
  ...
  const fetchOrder = async () => {
    ...
    try {
      ...
      let productOrdersForForm: any[] = [];
      
      if (orderData.productOrders) {
        if (Array.isArray(orderData.productOrders)) {
          productOrdersForForm = orderData.productOrders;
        } else {
          productOrdersForForm = Object.values(orderData.productOrders);
        }
      }
      
      const formData = {
        productOrders: productOrdersForForm.length > 0 
          ? productOrdersForForm.map((po: any) => ({
              product: po.product?.id?.toString() || '',
  ...
  const onSubmit = async (data: OrderUpdateData) => {
    ...
    // Same validation as CreateOrderPage, with proper null checking
    ...
    try {
      // Same data preparation as CreateOrderPage
      const orderData = {
  ...
  if (isFetchingOrder) {
    return (
      <Layout title="Modification de la Commande" subtitle="Chargement...">
  ...
  if (!order) {
    return (
      <Layout title="Modification de la Commande" subtitle="Commande introuvable">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
          La commande demandée n'existe pas ou n'a pas pu être chargée.
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout title="Modification de la Commande" subtitle={`Commande #${order.orderNumber}`}>
      {error && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {isLoading ? 'Mise à jour en cours...' : 'Mettre à jour la commande'}
          </button>
        </div>
      </form>
    </Layout>
